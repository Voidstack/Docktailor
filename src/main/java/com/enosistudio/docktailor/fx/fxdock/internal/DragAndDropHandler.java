...
public class DragAndDropHandler {
    ...
    protected static void setDropOp(ADockDropOperation op) {
        if (dockDropOperation != null) {
            if (dockDropOperation.isSame(op)) {
                ...
                dockDropOperation.removeHighlights();
            }
        }

        dockDropOperation = op;
    ...
    protected static ADockDropOperation createDropOnPane(FxDockPane client, Pane target, double screenx, double screeny) {
        ...
        EWhere where = determineWhere(x, y, x1, x2, y1, y2, w, h);
        ...
        addRectangleToDropOperation(op, target, where, x1, x2, y1, y2, w, h);
        return op;
    }
     ...
     * @param where  The location on the pane where the rectangle should be added (e.g., BOTTOM, CENTER, LEFT, RIGHT,
     *               TOP).
     ...
     */
    private static void addRectangleToDropOperation(ADockDropOperation op, Pane target, EWhere where, double x1, double x2, double y1, double y2, double w, double h) {
        switch (where) {
            case BOTTOM:
