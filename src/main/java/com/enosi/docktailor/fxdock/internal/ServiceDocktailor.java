...
import com.enosi.docktailor.fx.FxMenuBar;
import com.enosi.docktailor.fx.FxMenuItem;
import com.enosi.docktailor.fxdock.FxDockWindow;
import com.enosi.docktailor.common.Lazy;
...
public class ServiceDocktailor {
    ...
    public void initFXMenuBar(FxMenuBar fxMenuBar, FxDockWindow fxDockWindow) {
        Menu fxMenu = fxMenuBar.menu("Affichages");
        ...
        for (Class<? extends IControllerDockPane> draggableTab : ServiceDocktailor.getInstance().getFilteredDraggableTabsLazy().get()) {
            try {
                ...
                FxMenuItem m = new FxMenuItem(instanceDraggable.getTabName(), () -> fxDockWindow.addDockPane(instanceDraggable.createDockPane()));
                m.getStyleClass().add("item-gap");
                fxMenu.getItems().add(m);
            } catch (Exception e) {
                log.error("ServiceDocktailor : ", e);
            }
        }
