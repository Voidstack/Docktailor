...
/**
 * This class creates a chain of listeners which monitor the parent window 'showing' status.
 * <p>
 * scene.window -> window -> window.showing
 * <p>
 * This class simplifies code when action needs to be taken when the Node is added to a visible parent Window, or when
 * that Window is closed (hidden, as there is no destroy() method in FX).
 */
public class ParentWindow {
    ...
    public ParentWindow(Node n) {
        sceneListener = (observable, prev, cur) -> handleSceneChange(cur);
    ...
    protected ChangeListener<Boolean> createShowingListener(Window w) {
        return (observable, prev, cur) -> handleShowingChange(w, cur);
    }

    protected void handleSceneChange(Scene cur) {
        update(cur);
    }
